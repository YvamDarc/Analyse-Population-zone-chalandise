...
st.title("🗺️ Zone B — sélection communes (rayon)")
...
if st.button("🔎 Rechercher"):
    if q.strip():
        st.session_state["zone_b_geo"] = geocode_search(q.strip(), limit=8)
    else:
        st.warning("Entre une ville/adresse.")

results = st.session_state.get("zone_b_geo", [])
if not results:
...
if st.button("📍 Charger communes du rayon"):
    ...
    st.session_state["zone_b_in_radius"] = in_radius

in_radius = st.session_state.get("zone_b_in_radius", None)
...
# sélection
if "zone_b_selected_codes" not in st.session_state:
    st.session_state["zone_b_selected_codes"] = set()

view = in_radius[["code_insee", "nom", "codes_postaux", "code_dept", "population_geoapi", "dist_km"]].copy()
view["ajouter"] = view["code_insee"].isin(st.session_state["zone_b_selected_codes"])
